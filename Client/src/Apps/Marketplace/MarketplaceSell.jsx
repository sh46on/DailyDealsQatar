...
/* ─── Style injection (singleton, runs once) ─────────────────── */
let stylesInjected = false;
function InjectStyles() {
  useEffect(() => {
    if (stylesInjected || document.getElementById("msp-global-css")) return;
...
const Icon = {
  ...
  Arrow: memo(() => (
    <svg {...SVG_PROPS} strokeWidth="2.5">
      <path d="M5 12h14M12 5l7 7-7 7" />
    </svg>
  )),
  Spin: memo(() => (
    <svg {...SVG_PROPS} strokeWidth="2.5" style={{ display: "block", animation: "msp-spin 0.7s linear infinite" }}>
...
/* ─── Ad Slot ────────────────────────────────────────────────── */
const AdSlot = memo(function AdSlot({ variant = "leaderboard" }) {
...
const StatusChip = memo(function StatusChip({ type, label }) {
...
/* ─── Form Styles (static, defined once) ────────────────────── */
const FIELD_STYLE = {
...
/* ─── Form Fields ────────────────────────────────────────────── */
const FormFields = memo(function FormFields({ data, onChange }) {
  return (
    <>
      <div style={{ marginBottom: 18 }}>
        <label style={LABEL_STYLE}>Product Title *</label>
        <input className="msp-field-input" name="title" placeholder="e.g. iPhone 14, 128 GB Midnight"
          value={data.title} onChange={onChange} style={FIELD_STYLE} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 18 }} className="msp-row">
        <div>
          ...
          <input className="msp-field-input" name="price" type="number" min="0" placeholder="0.00"
            value={data.price} onChange={onChange} style={FIELD_STYLE} />
        </div>
        <div>
          <label style={LABEL_STYLE}>City *</label>
          <input className="msp-field-input" name="city" placeholder="e.g. Doha"
            value={data.city} onChange={onChange} style={FIELD_STYLE} />
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 18 }} className="msp-row">
        <div>
          <label style={LABEL_STYLE}>Condition</label>
          <select name="condition" value={data.condition} onChange={onChange} style={{ ...FIELD_STYLE, ...SELECT_EXTRA }}>
        ...
        <div>
          ...
          <input className="msp-field-input" name="category" placeholder="e.g. Electronics"
            value={data.category} onChange={onChange} style={FIELD_STYLE} />
        </div>
      </div>
      <div style={{ marginBottom: 18 }}>
        <label style={LABEL_STYLE}>Negotiable</label>
        <select name="is_negotiable" value={data.is_negotiable} onChange={onChange} style={{ ...FIELD_STYLE, ...SELECT_EXTRA }}>
      ...
      <div>
        ...
        <textarea className="msp-field-input" name="description"
          placeholder="Describe condition, features, reason for selling…"
          value={data.description} onChange={onChange} rows={4}
...
/* ─── Product Item ───────────────────────────────────────────── */
const ProductItem = memo(function ProductItem({ p, idx, onEdit, onDelete, onToggle }) {
  ...
  return (
    <div className="msp-item" style={{
      background: "#fff", borderRadius: 22,
      boxShadow: "0 4px 32px rgba(21,101,192,0.07), 0 1px 4px rgba(0,0,0,0.03)",
      border: "1.5px solid #e0ecfb",
      display: "grid", gridTemplateColumns: "6px 100px 1fr auto",
      alignItems: "stretch", overflow: "hidden", minHeight: 110,
      animation: "msp-fadeUp 0.4s cubic-bezier(0.22,1,0.36,1) both",
      animationDelay: `${idx * 0.05}s`,
    }}>
      {/* Left accent bar */}
      <div style={{ width: 6, background: BAR_COLORS[p.condition] || BAR_COLORS.used, borderRadius: "6px 0 0 6px", flexShrink: 0 }} />
      ...
      <div style={{ position: "relative", overflow: "hidden", background: BLUELT }}>
        {p.image_urls?.[0]
          ...
          : <div style={{
              ...
              <span style={{ width: 28, height: 28, color: BLUE, opacity: 0.5 }}><Icon.Box /></span>
            </div>
        }
        {p.category_name && (
          <span style={{
      ...
      <div style={{ padding: "14px 16px 12px", display: "flex", flexDirection: "column", justifyContent: "space-between", gap: 8, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 6 }}>
            ...
            {p.city && (
              <span style={{
                ...
                <span style={{ width: 10, height: 10, flexShrink: 0 }}><Icon.MapPin /></span>
                {p.city}
              </span>
            )}
          </div>
        ...
        <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
          <button className="msp-btn" onClick={handleEdit} style={{
            display: "inline-flex", alignItems: "center", gap: 5,
          ...
          <button className="msp-btn" onClick={handleDelete} style={{
            display: "inline-flex", alignItems: "center", gap: 5,
            fontFamily: FONT, fontSize: 11, fontWeight: 700,
          ...
          <button className="msp-btn" onClick={handleToggle} style={{
            display: "inline-flex", alignItems: "center", gap: 5,
            fontFamily: FONT, fontSize: 11, fontWeight: 700,
...
/* ─── Summary Card ───────────────────────────────────────────── */
const SUMMARY_ROWS = [
  { label: "Active",   dot: "#16a34a", key: "active"   },
  { label: "Inactive", dot: "#e11d48", key: "inactive" },
  { label: "New",      dot: "#3b82f6", key: "new"      },
  { label: "Used",     dot: "#f59e0b", key: "used"     },
];

const SummaryCard = memo(function SummaryCard({ products }) {
  const stats = useMemo(() => {
    ...
    return {
      ...
      new: newItems,
      used: products.length - newItems,
      total: products.length || 1,
    };
  }, [products]);

  const activePct = Math.min(100, (stats.active / stats.total) * 100);

  return (
    <div style={s.infoCard}>
      <div style={s.infoHead}>
        <span style={{ width: 20, height: 20, color: BLUE, flexShrink: 0 }}><Icon.Box /></span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {SUMMARY_ROWS.map(row => (
          <div key={row.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <span style={{ fontFamily: FONT, fontSize: 16, fontWeight: 800, color: "#0f172a" }}>{stats[row.key]}</span>
          </div>
        ))}
      </div>
...
const PANEL_HEADER_BASE = {
  background: "linear-gradient(130deg, #0f3460 0%, #1565c0 50%, #1976d2 100%)",
  padding: "20px 26px",
  position: "relative", overflow: "hidden",
};

const PanelHeader = memo(function PanelHeader({ icon, title, badge, actions }) {
  return (
    <div style={{ ...PANEL_HEADER_BASE, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      <div style={DOTS_BG} />
      <div style={{ display: "flex", alignItems: "center", gap: 12, position: "relative" }}>
        <span style={{ width: 18, height: 18, color: "rgba(255,255,255,0.88)", flexShrink: 0 }}>{icon}</span>
        <h2 style={{ fontFamily: FONT_D, fontSize: 18, fontWeight: 900, color: "#fff", letterSpacing: "-0.02em" }}>{title}</h2>
      </div>
      {badge != null && (
        <span style={{
...
/* ─── Edit Modal (lazily rendered, not lazy-loaded — it's small) */
const EditModal = memo(function EditModal({ target, form, loading, onChange, onSave, onClose }) {
  const canSave = !loading && !!form.title && !!form.price;
  ...
  return (
    ...
    >
      <div role="dialog" aria-modal="true" aria-label={`Edit ${target.title}`} style={{
        ...
        {/* Header */}
        <div style={{ ...PANEL_HEADER_BASE, display: "flex", alignItems: "center", justifyContent: "space-between", borderRadius: "24px 24px 0 0", position: "sticky", top: 0, zIndex: 2 }}>
          <div style={DOTS_BG} />
          <div style={{ display: "flex", alignItems: "center", gap: 10, position: "relative" }}>
            <span style={{ width: 17, height: 17, color: "rgba(255,255,255,0.88)" }}><Icon.Pencil /></span>
            <h3 style={{ fontFamily: FONT_D, fontSize: 18, fontWeight: 900, color: "#fff", letterSpacing: "-0.02em" }}>Edit Listing</h3>
          </div>
          <button className="msp-modal-close" onClick={onClose} aria-label="Close" style={{
        ...
        <div style={{ padding: "24px 24px 8px" }}>
          <FormFields data={form} onChange={onChange} />
        </div>

        {/* Footer */}
        <div style={{
          display: "flex", gap: 10, justifyContent: "flex-end",
          padding: "18px 24px 24px", borderTop: "1px solid #e0ecfb", background: "#f7fbff",
        }} className="msp-modal-footer">
          <button className="msp-btn-cancel" onClick={onClose} style={{
            padding: "10px 22px", background: "#fff", color: "#64748b",
            border: "1.5px solid #e0ecfb", borderRadius: 12,
            fontFamily: FONT, fontSize: 13, fontWeight: 600, cursor: "pointer",
          }}>Cancel</button>
          <button className="msp-btn-save" onClick={onSave} disabled={!canSave} style={{
            ...
            <span style={{ width: 14, height: 14 }}>{loading ? <Icon.Spin /> : <Icon.Save />}</span>
...
/* ─── Add Product Panel ──────────────────────────────────────── */
const AddProductPanel = memo(function AddProductPanel({ form, onChange, onSubmit, submitting, previews, onImages, fileRef }) {
  const canSubmit = !submitting && !!form.title && !!form.price;
  return (
    <div style={{
      background: "#fff", borderRadius: 22,
      ...
      <PanelHeader icon={<Icon.Plus />} title="New Listing" />
      ...
      <div style={{ padding: "28px 26px 24px" }}>
        ...
        <div style={{ marginTop: 18 }}>
          ...
          <div style={{
            ...
            <input type="file" multiple accept="image/*" ref={fileRef} onChange={onImages}
        ...
        <button className="msp-submit" onClick={onSubmit} disabled={!canSubmit} style={{
          ...
          <span style={{ width: 17, height: 17 }}>{submitting ? <Icon.Spin /> : <Icon.Arrow />}</span>
...
/* ─── Listings Panel ─────────────────────────────────────────── */
const ListingsPanel = memo(function ListingsPanel({ products, loading, onEdit, onDelete, onToggle }) {
  const count = products.length;
  const badgeLabel = !loading && count > 0 ? `${count} listing${count !== 1 ? "s" : ""}` : undefined;
  ...
  return (
    <div style={{
      ...
      <PanelHeader icon={<Icon.List />} title="My Listings" badge={badgeLabel} />
      ...
      <div style={{ padding: "20px 22px 22px" }}>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "56px 20px", animation: "msp-fadeUp 0.45s ease both" }}>
            <div style={{ width: 56, height: 56, color: BLUE, opacity: 0.25, margin: "0 auto 16px", animation: "msp-floatOrb 4s ease-in-out infinite" }}>
              <Icon.Box />
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {products.map((p, i) => (
              <ProductItem key={p.id} p={p} idx={i} onEdit={onEdit} onDelete={onDelete} onToggle={onToggle} />
            ))}
          </div>
        )}
...
/* ─── Stat Pills (hero) ──────────────────────────────────────── */
const STAT_DOTS = { listings: "#93c5fd", active: "#16a34a", inactive: "#e11d48" };
...
export default function MarketplaceSell() {
  ...
  const { productCount, activeCount } = useMemo(() => ({
  ...
  const showToast = useCallback((msg) => {
    ...
    const id = setTimeout(() => setToast({ on: false, msg: "" }), 3000);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      load();
      startTransition(() => setTab("listings"));
    } catch {
  ...
  /* ── Tab change with concurrent-mode transition ── */
  ...
  const statPills = useMemo(() => [
    { key: "listings", dot: STAT_DOTS.listings, num: productCount,               label: "Listings" },
    { key: "active",   dot: STAT_DOTS.active,   num: activeCount,                label: "Active"   },
    { key: "inactive", dot: STAT_DOTS.inactive, num: productCount - activeCount, label: "Inactive" },
  ...
  return (
    <MarketplaceLayout>
      ...
      <div style={{ minHeight: "100vh", background: "#f0f6ff", fontFamily: FONT }}>
        ...
        <div style={s.hero}>
          ...
          <svg style={s.heroWave} viewBox="0 0 1440 60" preserveAspectRatio="none" aria-hidden="true">
            <path style={{ animation: "msp-waveSlide 10s linear infinite" }}
              d="M0,30 C240,55 480,5 720,30 C960,55 1200,5 1440,30 C1680,55 1920,5 2160,30 L2160,60 L0,60 Z"
              fill="#f0f6ff" />
            <path style={{ animation: "msp-waveSlide 15s linear infinite reverse" }}
          ...
          <div style={s.heroInner}>
            ...
            {!listLoading && productCount > 0 && (
              <div style={s.statRow}>
                {statPills.map(({ key, dot, num, label }) => (
                  <div key={key} className="msp-stat-pill" style={s.statPill}>
        ...
        {/* ── BODY ── */}
        <div style={s.body}>
          <div style={{ marginBottom: 24 }}><AdSlot variant="leaderboard" /></div>
          ...
          <div className="msp-layout" style={s.layout}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{
                ...
                {[
                  ...
                  return (
                    <button key={key}
                      className={`msp-tab${isActive ? " msp-tab--active" : ""}`}
                      onClick={() => handleTabChange(key)}
                      style={{
                        flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                        padding: "11px 20px", borderRadius: 18, border: "none",
                        fontFamily: FONT, fontSize: 13, fontWeight: 700,
                        background: isActive ? "linear-gradient(130deg, #0f3460, #1565c0)" : "transparent",
                        color: isActive ? "#fff" : "#64748b",
                        boxShadow: isActive ? "0 4px 14px rgba(21,101,192,0.30)" : "none",
                        whiteSpace: "nowrap",
                      }}
                    >
                      ...
                      {badge !== undefined && (
                        <span style={{
                          ...
                          background: isActive ? "rgba(255,255,255,0.22)" : BLUELT,
            ...
            <aside className="msp-sidebar" style={s.sidebar}>
              ...
              <div style={s.infoCard}>
                <div style={s.infoHead}>
                  <span style={{ width: 20, height: 20, color: BLUE, flexShrink: 0 }}><Icon.Lightbulb /></span>
              ...
              <AdSlot variant="rectangle" />
              <div style={{ marginTop: 16 }}><AdSlot variant="rectangle" /></div>
            </aside>
          </div>

          <div style={{ marginTop: 32 }}><AdSlot variant="leaderboard" /></div>
        </div>
      </div>

      {/* ── EDIT MODAL (conditionally rendered = lazy in effect) ── */}
      {editTarget && (
        <EditModal
          target={editTarget}
          form={editForm}
      ...
      {/* ── TOAST ── */}
      <div className={`msp-toast${toast.on ? " on" : ""}`} role="status" aria-live="polite">
...
/* ─── Static data (outside component = no realloc) ──────────── */
const SELLER_TIPS = [
