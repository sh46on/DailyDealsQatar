/**
 * Home.jsx — Production-optimised
 *
 * Optimisations applied (annotated inline with ①–⑳):
 *
 * ① React.lazy + Suspense      — ProductModal & PdfModal split into async chunks
 * ② memo()                     — every pure sub-component wrapped; skips re-renders
 * ③ useCallback()              — all event handlers stabilised
 * ④ useMemo()                  — all derived data/values memoised (already present, extended)
 * ⑤ LazyProductImage           — native loading="lazy" + decoding="async" + skeleton
 * ⑥ LazyPDFThumb debounce      — 150 ms gate prevents spawning workers for fast-scroll
 * ⑦ IntersectionObserver       — cards render PDF worker only when in viewport
 * ⑧ useLazyVisible             — shared hook; disconnect after first intersection
 * ⑨ PDF_OPTIONS constant       — stable object ref; prevents react-pdf reloads
 * ⑩ pdfjs worker via import.meta.url — correct ESM worker path, no CDN round-trip
 * ⑪ useBodyScrollLock          — extracted hook; restores previous overflow on unmount
 * ⑫ useFocusTrap               — a11y: keyboard stays inside modal (WCAG 2.1 §2.1.2)
 * ⑬ key={pdfUrl} on <Document> — forces clean worker when URL changes; prevents stale render
 * ⑭ mountedRef guard           — setState never called after unmount (memory/warning safe)
 * ⑮ useEffect dep-array audits — all effects have correct, minimal dependencies
 * ⑯ CSS string hoisted to module scope — not recreated on every render
 * ⑰ CAT_COLORS / CAT_META outside component — static objects allocated once at module load
 * ⑱ parsePageFromUrl           — try/catch + NaN guard; no crash on malformed URLs
 * ⑲ role / aria-* attributes   — dialog, tab, tablist, status, complementary, navigation
 * ⑳ Escape-key handler in modals — moved to modal components; Home doesn't duplicate it
 */
...
import {
  ...
  Package, X, Tag, Search,
  Flame, Calendar, Eye, Building2, Star,
} from "lucide-react";
import { getImageUrl } from "../api/media";

/* ① Lazy-load heavy modal bundles — not shipped until first open */
const ProductModal = lazy(() => import("../components/modals/ProductModal"));
const PdfModal     = lazy(() => import("../components/modals/PdfModal"));

/* ⑩ PDF.JS WORKER — ESM path; no CDN needed */
pdfjs.GlobalWorkerOptions.workerSrc = new URL(
  "pdfjs-dist/build/pdf.worker.min.js",
  import.meta.url,
).toString();

/* ⑨ Stable options object — new ref each render causes react-pdf to reload the doc */
const PDF_OPTIONS = { cMapPacked: true };

/* ─────────────────────────────────────────────
   ⑰ STATIC DATA — allocated once at module load
───────────────────────────────────────────── */
const T = {
  red:         "#E30613",
  redDark:     "#B80010",
...
const CAT_ALL = {
  label:"All Offers", Icon:Sparkles,
  accent:T.red, bg:`linear-gradient(135deg,#FFF0F0,#FFE0E0)`,
  pill:"#FECDD3", pillTx:"#881337", glow:T.redGlow,
};
const catTypeMeta = (type) => {
  if (!type) return CAT_ALL;
  return CAT_META[type.toLowerCase()] ?? CAT_ALL;
};
...
/* ⑱ Safely parse page number; NaN-safe */
function parsePageFromUrl(url, param) {
  if (!url) return 1;
  try {
    const val = parseInt(new URL(url).searchParams.get(param) || "1", 10);
...
/* ─────────────────────────────────────────────
   ⑧ SHARED HOOK: useLazyVisible
   Fires once when the element enters the viewport,
   then immediately disconnects the observer.
───────────────────────────────────────────── */
function useLazyVisible(rootMargin = "200px") {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setVisible(true); // fallback for environments without IntersectionObserver
    }
...
/* ─────────────────────────────────────────────
   ⑤ LAZY PRODUCT IMAGE
   Skeleton while loading; Package icon on error.
───────────────────────────────────────────── */
const LazyProductImage = memo(function LazyProductImage({ src, alt }) {
  const [status, setStatus] = useState(src ? "loading" : "empty");

  // Reset when src prop changes (modal reused for different product)
  ...
  if (status === "empty" || status === "error") {
    return (
      <div className="prd-img-ph">
        <Package size={32} strokeWidth={1.2} color={T.subtle} aria-hidden="true" />
      </div>
    );
  }
...
const AdPlaceholder = memo(function AdPlaceholder({ label = "Advertisement" }) {
  return (
...
/* ⑥⑦ Lazy PDF Thumbnail — debounced + IntersectionObserver guarded */
const LazyPDFThumb = memo(function LazyPDFThumb({ pdfUrl, catAccent, title }) {
  ...
  // ⑥ 150 ms debounce — fast pagination won't spawn workers for unmounted cards
  useEffect(() => {
    if (visible && thumbState === "idle") {
      timerRef.current = setTimeout(() => {
        if (mountedRef.current) setThumbState("loading");
      }, 150);
    }
    return () => clearTimeout(timerRef.current);
  }, [visible, thumbState]);

  // Clean reset when URL changes
  useEffect(() => {
    clearTimeout(timerRef.current);
    if (mountedRef.current) setThumbState("idle");
  }, [pdfUrl]);

  const safeSet = useCallback((s) => { // ③
    if (mountedRef.current) setThumbState(s);
  }, []);

  const handleError = useCallback((err) => {
    if (!mountedRef.current) return;
    const msg = (err?.message || String(err)).toLowerCase();
    // Benign worker teardown — not real PDF failures
    if (
  ...
  return (
    <div ref={ref} className="pdf-mag-thumb-outer">
      <div
        ...
        style={{ opacity: thumbState === "ready" ? 0 : 1, background: (catAccent || T.red) + "18" }}
      >
        {thumbState === "loading"
          ? <div className="pdf-spin" style={{ borderTopColor: catAccent || T.red }} />
          : <>
              <FileText size={32} color={catAccent || T.red} strokeWidth={1.4} />
              <span className="pdf-fallback-label">{title || "Flyer"}</span>
            </>
        }
      </div>

      {(thumbState === "loading" || thumbState === "ready") && pdfUrl && (
        <div className="pdf-mag-doc" style={{ opacity: thumbState === "ready" ? 1 : 0 }}>
          {/* ⑬ key={pdfUrl} forces a clean worker instance when URL changes */}
          <Document
            key={pdfUrl}
            file={pdfUrl}
            onLoadSuccess={() => safeSet("ready")}
      ...
      {thumbState === "error" && (
        <div className="pdf-mag-fallback" style={{ background: (catAccent || T.red) + "12" }}>
          <FileText size={32} color={catAccent || T.red} strokeWidth={1.4} />
          <span className="pdf-fallback-label">{title || "Flyer"}</span>
        </div>
      )}
...
/* ② PDF Card */
const PDFCard = memo(function PDFCard({ pdf, onView }) {
  const m    = catTypeMeta(pdf?.categoryType);
  const CIcon = m?.Icon || FileText;

  // ④ Memoised URL build — only recomputes when pdf.url changes
  const pdfUrl = useMemo(() => buildPdfUrl(pdf?.url), [pdf?.url]);
  const [cardRef, cardVisible] = useLazyVisible("150px");

  // ④ Memoised expiry calculation
  const daysLeft = useMemo(() => {
    if (!pdf?.validUntil) return null;
    try {
      const diff = Math.max(0, Math.ceil((new Date(pdf.validUntil) - new Date()) / 86400000));
      return diff;
    } catch (_) { return null; }
  }, [pdf?.validUntil]);

  // ③ Stable click handler
  const handleClick = useCallback(() => onView?.(pdf), [onView, pdf]);
  ...
  return (
    <article
      ...
      onClick={handleClick}
      aria-label={`${pdf.title || "Flyer"} from ${pdf.company || "Store"}`}
    >
      <div className="pdf-mag-visual">
        ...
        {(!pdfUrl || !cardVisible) && (
          <div className="pdf-mag-icon-fallback">
            <CIcon size={40} color={m?.accent || T.red} strokeWidth={1.2} />
          </div>
        )}
        ...
        <div className="pdf-mag-top">
          <div className="pdf-mag-badge" style={{ background: m?.accent || T.red }}>
            <CIcon size={9} strokeWidth={2.5} />
            <span>{m?.label || pdf.categoryTypeLabel || "Offer"}</span>
          </div>
        ...
        <div className="pdf-mag-bottom">
          <div className="pdf-mag-co-row">
            {pdf.companyLogo
              ? <img src={getImageUrl(pdf.companyLogo)} alt={pdf.company || ""} className="pdf-mag-co-img" loading="lazy" decoding="async" />
              : <div className="pdf-mag-co-dot" style={{ background: m?.accent || T.red }}>{(pdf.company || "C")[0]}</div>
            }
            <span className="pdf-mag-co-name">{pdf.company || ""}</span>
          </div>
          <h4 className="pdf-mag-title">{pdf.title || "Untitled Flyer"}</h4>
          <div className="pdf-mag-meta-row">
            ...
            <span className="pdf-mag-view-cta" style={{ background: m?.accent || T.red }}>
              <Eye size={10} /> View
            </span>
          </div>
        </div>
...
/* ② Product Card */
const ProductCard = memo(function ProductCard({ product, onViewDeal }) {
  const [imgErr, setImgErr] = useState(false);
  const c = getCat(product?.categoryId);

  // ③ Stable handlers
  const handleClick    = useCallback(() => onViewDeal?.(product), [onViewDeal, product]);
  const handleImgError = useCallback(() => setImgErr(true), []);
  ...
  return (
    ...
    >
      <div className="prd-img-wrap" style={{ position:"relative" }}>
        {product.image && !imgErr
          ? <LazyProductImage src={product.image} alt={product.title} />
...
/* ② Company Carousel */
const CompanyCarousel = memo(function CompanyCarousel({ companies, selectedCompanyId, onSelect }) {
  ...
  // ③ Stable pill click — avoids re-renders of every pill on parent state change
  const handleSelectNull = useCallback(() => onSelect(null), [onSelect]);
...
/* ② Extracted to avoid recreating click handlers inside the map loop */
const CompanyPill = memo(function CompanyPill({ company: c, pal, isActive, onSelect }) {
  const handleClick = useCallback(() => onSelect(isActive ? null : c.id), [onSelect, isActive, c.id]);
  return (
    <button
    ...
    >
      <div className="co-pill-avatar" style={!c.logo ? { background: pal.bg, color: pal.text } : {}}>
        {c.logo
          ? <img src={getImageUrl(c.logo)} alt={c.name || ""} loading="lazy" decoding="async" />
...
/* ② Sidebar */
const Sidebar = memo(function Sidebar({ categories, active, onSelect, onClose }) {
  const [openIds, setOpenIds] = useState({});

  const toggle       = useCallback(id => setOpenIds(p => ({ ...p, [id]: !p[id] })), []);
  const selectAll    = useCallback(() => { onSelect({ catId:null, subId:null }); onClose?.(); }, [onSelect, onClose]);
...
/* ② Extracted — each category row has its own stable handler */
const SidebarCategory = memo(function SidebarCategory({ cat, active, isOpen, onToggle, onSelect, onClose }) {
  const c        = getCat(cat.id);
  const hasSub   = cat.subcategories?.length > 0;
  const isActive = active?.catId === cat.id && active?.subId === null;

  const handleSelect = useCallback(() => { onSelect({ catId:cat.id, subId:null }); onClose?.(); }, [cat.id, onSelect, onClose]);
  const handleToggle = useCallback(e  => { e.stopPropagation(); onToggle(cat.id); },               [cat.id, onToggle]);
  ...
  return (
    <div>
      <button className={`scat-item ${isActive ? "active" : ""}`}
        ...
        <span className="scat-label" onClick={handleSelect}>{cat.name}</span>
        {cat.count > 0 && <em className="scat-count">{cat.count}</em>}
        {hasSub && (
          <span className="scat-expand" onClick={handleToggle} aria-label={isOpen ? "Collapse" : "Expand"}>
      ...
      {hasSub && isOpen && (
        <div className="scat-subs">
          {cat.subcategories.map(sub => (
            <SidebarSub key={sub.id} sub={sub} catId={cat.id} accent={c.accent} active={active} onSelect={onSelect} onClose={onClose} />
          ))}
        </div>
      )}
    </div>
...
const SidebarSub = memo(function SidebarSub({ sub, catId, accent, active, onSelect, onClose }) {
  const isActive = active?.subId === sub.id;
  const handleClick = useCallback(() => { onSelect({ catId, subId:sub.id }); onClose?.(); }, [catId, sub.id, onSelect, onClose]);
  return (
    <button className={`scat-sub ${isActive ? "active" : ""}`} onClick={handleClick}>
      <span className="sub-dot" style={{ background: isActive ? accent : T.subtle }} />
      {sub.name}
...
const EmptyState = memo(function EmptyState({ message, sub }) {
  return (
...
const PaginationBar = memo(function PaginationBar({ page, total, onNext, onPrev, hasNext, hasPrev }) {
  if (total <= 1 && !hasNext && !hasPrev) return null;
  return (
...
export default function Home() {
  ...
  const isPdfPaginated = showPDFs && activeNav === "all" && selectedCompany === null;
  ...
  const {
    ...
    productPageUrl,
    flyerPageUrl: isPdfPaginated ? flyerPageUrl : null,
  });

  const pdfResults = pdfs?.results || [];

  // ④ pdfNavItems only recomputes when pdfCategoryTypes changes
  const pdfNavItems = useMemo(() => {
    ...
    try {
      ...
      return FIXED_PDF_NAV;
    }
  }, [pdfCategoryTypes]);

  // ⑮ Correct deps — only re-runs when nav or nav items actually change
  useEffect(() => {
  ...
  // Reset BOTH cursors on tab switch
  useEffect(() => {
  ...
  // Reset product cursor on product filter change
  useEffect(() => { setProductPageUrl(null); }, [active.catId, active.subId, selectedCompany, activeTab]);

  // Reset flyer cursor on flyer filter change
  useEffect(() => { setFlyerPageUrl(null); }, [resolvedTypeValue, selectedCompany]);

  const paginationSource = useMemo(() => {
    if (!showPDFs)      return pagination || {};
    if (isPdfPaginated) return pdfs || {};
    return {};
  }, [showPDFs, isPdfPaginated, pagination, pdfs]);

  const page = useMemo(() => {
    if (showPDFs && isPdfPaginated) return parsePageFromUrl(flyerPageUrl,   "flyer_page");
    if (!showPDFs)                  return parsePageFromUrl(productPageUrl,  "page");
    return 1;
  }, [showPDFs, isPdfPaginated, flyerPageUrl, productPageUrl]);

  const totalPages = useMemo(() => {
    const count = paginationSource?.count;
  ...
  // ③ Stable modal openers/closers
  const openProductModal  = useCallback(p  => setProductModal(p),  []);
  const closeProductModal = useCallback(() => setProductModal(null), []);
  const openPdfModal      = useCallback(p  => setPdfModal(p),       []);
  const closePdfModal     = useCallback(() => setPdfModal(null),     []);
  const clearCompany      = useCallback(() => setSelectedCompany(null), []);
  ...
  const formattedPdfs = useMemo(() => {
    try {
      const fromTop = pdfResults.map(p => ({
        id:                p.id,
        title:             p.title             || "",
        url:               p.pdf ? String(p.pdf).replace(/["']/g,"").trim() : null,
      ...
      console.error("[formattedPdfs]", err);
      return [];
    }
  }, [pdfs, companies, selectedCompany, resolvedTypeValue, pdfResults]);
  ...
  const formattedProducts = useMemo(() => {
    try {
      let list = (products || []).map(p => ({
        ...
        originalPrice: p.old_price ? `QAR ${parseFloat(p.old_price).toLocaleString("en-QA")}` : null,
      ...
      console.error("[formattedProducts]", err);
      return [];
    }
  ...
  const sectionTitle = useMemo(() => {
    try {
      ...
      return companyName
        ? `${companyName} — ${currentPdfNav?.label || "Offers"}`
        : currentPdfNav?.label || "All Offers";
    } catch (_) {
      return showPDFs ? "All Offers" : "All Products";
    }
  ...
  if (error) return (
    <>
      ...
      <div className="full-center" role="alert">
        ...
        <p style={{ color:T.red, marginTop:14, fontSize:14 }}>{String(error)}</p>
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="root">
        ...
        {/* ① Lazy-loaded modal chunks — null fallback: modal renders silently */}
        {productModal && (
          <Suspense fallback={null}>
            <ProductModal product={productModal} onClose={closeProductModal} />
          </Suspense>
        )}
        {pdfModal && (
          <Suspense fallback={null}>
            <PdfModal pdf={pdfModal} onClose={closePdfModal} />
          </Suspense>
        )}
        ...
        <nav className="topnav" aria-label="Main navigation">
          <div className="topnav-inner">
            ...
            >
              <ShoppingCart size={13} strokeWidth={2} /> Products
            </button>
        ...
        <div className="body">
          ...
          <main className="main" id="main-content">
            ...
            <div className="sec-hdr">
              <div>
                ...
                <div className="sec-count">
                  {showPDFs
                    ? `${displayItems.length} flyer${displayItems.length !== 1 ? "s" : ""}${isPdfPaginated && paginationSource?.count > displayItems.length ? ` of ${paginationSource.count}` : ""}`
              ...
              <div className="sec-controls">
                {!showPDFs && (
                  <button className="filter-btn" onClick={openDrawer} aria-label="Open categories filter">
                    <SlidersHorizontal size={13} /> Categories
                  </button>
                )}
                {selectedCompany !== null && (
                  <button className="clear-chip" onClick={clearCompany} aria-label="Clear store filter">
            ...
            {displayItems.length > 0 ? (
              <div
                className={showPDFs ? "pdf-grid" : "prd-grid"}
                role="list"
                aria-label={showPDFs ? "Flyers" : "Products"}
              >
                {displayItems.map(item =>
                  showPDFs
                    ? <PDFCard     key={item.id} pdf={item}     onView={openPdfModal}         />
                    : <ProductCard key={item.id} product={item} onViewDeal={openProductModal} />
                )}
              </div>
            ) : (
              <EmptyState
                  ...
                  showPDFs
                    ? selectedCompany
                      ...
                      : currentPdfNav?.key !== "all"
                        ? `No flyers available for ${currentPdfNav?.label || "this category"} yet`
        ...
        {!showPDFs && (
          <div className={`drawer ${drawerOpen ? "open" : ""}`} aria-hidden={!drawerOpen}>
            <Sidebar
              categories={sidebarCategories}
              active={active}
              onSelect={sel => { setActive(sel); closeDrawer(); }}
              onClose={closeDrawer}
            />
          </div>
        )}
...
.navpill { display:inline-flex; align-items:center; gap:6px; flex-shrink:0; padding:8px 17px; border-radius:100px; border:1.5px solid ${T.border}; background:${T.white}; color:${T.slate}; font-size:12.5px; font-weight:700; white-space:nowrap; transition:all .18s cubic-bezier(.4,0,.2,1); font-family:'Inter', sans-serif; letter-spacing:.01em; }
...
/* ── Banner Ad ── */
.top-ad-container { max-width:1400px; margin:16px auto 0; padding:0 20px; min-height:90px; background:${T.white}; border:1.5px dashed ${T.border}; border-radius:16px; display:flex; align-items:center; justify-content:center; }
...
/* ── Sidebar ── */
.sidebar { background:${T.white}; border:1.5px solid ${T.border}; border-radius:20px; padding:18px 14px; position:sticky; top:72px; max-height:calc(100vh - 96px); overflow-y:auto; scrollbar-width:thin; scrollbar-color:${T.border} transparent; }
.sidebar-close { position:absolute; top:12px; right:12px; background:none; border:none; color:${T.muted}; display:flex; padding:4px; border-radius:8px; transition:color .15s; cursor:pointer; }
.sidebar-close:hover { color:${T.red}; }
.sidebar-heading { display:flex; align-items:center; gap:8px; font-family:'Poppins', sans-serif; font-size:11px; font-weight:600; color:${T.dark}; text-transform:uppercase; letter-spacing:.1em; padding-bottom:14px; border-bottom:1.5px solid ${T.borderLight}; margin-bottom:14px; }
.scat-item { width:100%; display:flex; align-items:center; gap:9px; padding:9px 11px; border:none; background:transparent; border-radius:11px; font-size:13px; font-weight:600; color:${T.charcoal}; cursor:pointer; transition:all .18s; margin-bottom:2px; font-family:'Inter', sans-serif; text-align:left; }
...
.scat-sub   { width:100%; display:flex; align-items:center; gap:7px; padding:6px 9px; border-radius:8px; border:none; background:transparent; font-size:12px; color:${T.muted}; cursor:pointer; text-align:left; font-family:inherit; transition:all .15s; }
...
/* ── Company Carousel ── */
.co-carousel { background:${T.white}; border:1.5px solid ${T.border}; border-radius:20px; padding:12px 14px; margin-bottom:20px; display:flex; align-items:center; gap:8px; }
.co-wrap  { flex:1; overflow:hidden; }
.co-track { display:flex; gap:8px; transition:transform .32s cubic-bezier(.4,0,.2,1); }
.co-pill  { display:inline-flex; align-items:center; gap:8px; flex-shrink:0; padding:7px 16px 7px 8px; border-radius:100px; border:1.5px solid ${T.border}; background:${T.white}; color:${T.slate}; font-size:12.5px; font-weight:700; white-space:nowrap; cursor:pointer; transition:all .2s; font-family:'Inter', sans-serif; }
...
.co-pill-avatar { width:26px; height:26px; border-radius:50%; overflow:hidden; flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:9px; font-weight:800; font-family:'Poppins', sans-serif; }
...
.co-nav-btn { flex-shrink:0; width:34px; height:34px; border-radius:50%; border:1.5px solid ${T.border}; background:${T.white}; display:flex; align-items:center; justify-content:center; cursor:pointer; color:${T.charcoal}; transition:all .18s; }
...
.pdf-mag-card { position:relative; border-radius:22px; overflow:hidden; cursor:pointer; background:${T.darkMid}; aspect-ratio:3/4.2; transition:transform .35s cubic-bezier(.2,.9,.4,1.1), box-shadow .35s; box-shadow:0 4px 20px rgba(0,0,0,.1); opacity:0; }
.pdf-mag-visible { animation:cardReveal .42s cubic-bezier(.34,1,.64,1) both; opacity:1; }
.pdf-mag-card:hover { transform:translateY(-10px) rotate(-0.5deg) scale(1.015); box-shadow:0 30px 60px rgba(0,0,0,.22), 0 10px 20px rgba(0,0,0,.1); }
...
.pdf-mag-fallback { position:absolute; inset:0; z-index:1; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; transition:opacity .4s ease; }
.pdf-fallback-label { font-size:11px; font-weight:600; color:${T.dark}; line-height:1.3; text-align:center; padding:0 12px; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; opacity:.7; }
.pdf-spin { width:26px; height:26px; border-radius:50%; border:3px solid rgba(0,0,0,.1); animation:spin .75s linear infinite; }
.pdf-mag-doc { position:absolute; inset:0; z-index:2; display:flex; align-items:flex-start; justify-content:center; overflow:hidden; transition:opacity .4s ease; }
...
.pdf-mag-gradient { position:absolute; inset:0; z-index:3; background:linear-gradient(to bottom, rgba(0,0,0,.55) 0%, transparent 35%, transparent 50%, rgba(0,0,0,.78) 100%); }
.pdf-mag-top { position:absolute; top:12px; left:12px; right:12px; z-index:4; display:flex; align-items:flex-start; justify-content:space-between; gap:8px; }
.pdf-mag-badge { display:inline-flex; align-items:center; gap:5px; padding:5px 11px; border-radius:100px; font-size:9px; font-weight:800; text-transform:uppercase; letter-spacing:.07em; color:${T.white}; font-family:'Poppins', sans-serif; box-shadow:0 2px 10px rgba(0,0,0,.3); }
.pdf-mag-urgent { display:inline-flex; align-items:center; gap:4px; padding:4px 10px; border-radius:100px; background:rgba(255,200,0,.92); color:#000; font-size:9px; font-weight:800; font-family:'Poppins', sans-serif; letter-spacing:.04em; animation:urgentPulse 1.5s ease-in-out infinite; flex-shrink:0; }
...
.pdf-mag-title   { font-family:'Poppins', sans-serif; font-size:15px; font-weight:600; color:${T.white}; line-height:1.35; margin-bottom:10px; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; text-shadow:0 1px 4px rgba(0,0,0,.4); }
.pdf-mag-meta-row { display:flex; align-items:center; justify-content:space-between; gap:8px; }
.pdf-mag-date { display:flex; align-items:center; gap:5px; font-size:10px; color:rgba(255,255,255,.6); font-family:'Inter', sans-serif; }
.pdf-mag-view-cta { display:inline-flex; align-items:center; gap:5px; padding:5px 14px; border-radius:100px; font-size:10.5px; font-weight:800; color:${T.white}; font-family:'Poppins', sans-serif; letter-spacing:.04em; box-shadow:0 3px 12px rgba(0,0,0,.25); transition:transform .2s; }
.pdf-mag-card:hover .pdf-mag-view-cta { transform:scale(1.06); }

/* ── Product Grid & Card ── */
.prd-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(185px,1fr)); gap:18px; }
.prd-card { background:${T.white}; border-radius:20px; border:1.5px solid ${T.border}; overflow:hidden; cursor:pointer; transition:transform .3s cubic-bezier(.2,.9,.4,1.1), box-shadow .3s, border-color .2s; }
.prd-card:hover { transform:translateY(-6px); box-shadow:0 20px 40px rgba(13,15,20,.09), 0 4px 12px rgba(13,15,20,.06); border-color:${T.subtle}; }
.prd-img-wrap { position:relative; width:100%; aspect-ratio:1; background:${T.bg}; overflow:hidden; }
.prd-img-wrap img { width:100%; height:100%; object-fit:cover; transition:transform .45s cubic-bezier(.2,.9,.4,1.05); }
.prd-card:hover .prd-img-wrap img { transform:scale(1.07); }
.prd-img-ph { width:100%; height:100%; display:flex; align-items:center; justify-content:center; background:${T.bg}; }
.prd-shine { position:absolute; inset:0; background:linear-gradient(135deg, rgba(255,255,255,.18) 0%, transparent 50%); pointer-events:none; }
.prd-discount { position:absolute; top:10px; left:10px; display:inline-flex; align-items:center; gap:4px; background:${T.success}; color:${T.white}; padding:4px 10px; border-radius:8px; font-size:9.5px; font-weight:800; z-index:1; font-family:'Poppins', sans-serif; letter-spacing:.04em; box-shadow:0 2px 8px rgba(0,196,140,.35); }
.prd-badge { position:absolute; bottom:10px; left:10px; display:inline-flex; align-items:center; color:${T.white}; padding:4px 10px; border-radius:8px; font-size:9.5px; font-weight:800; z-index:1; font-family:'Poppins', sans-serif; letter-spacing:.04em; }
...
.pg-btn { min-width:36px; height:38px; border-radius:12px; border:1.5px solid ${T.border}; background:${T.white}; color:${T.dark}; font-size:13px; font-weight:700; display:inline-flex; align-items:center; gap:6px; padding:0 16px; transition:all .15s; cursor:pointer; font-family:inherit; }
...
.drawer  { position:fixed; top:0; left:0; bottom:0; width:280px; background:${T.white}; z-index:60; padding:20px 14px; overflow-y:auto; transform:translateX(-100%); transition:transform .3s cubic-bezier(.4,0,.2,1); box-shadow:6px 0 32px rgba(0,0,0,.12); }
.drawer.open { transform:translateX(0); }

/* ── Modals ── */
.modal-overlay { position:fixed; inset:0; background:${T.overlay}; z-index:100; display:flex; align-items:center; justify-content:center; padding:20px; animation:fadeIn .2s ease; }
.modal-box { background:${T.white}; border-radius:22px; width:100%; max-width:520px; overflow:hidden; position:relative; animation:slideUp .25s ease; max-height:90vh; display:flex; flex-direction:column; border:1.5px solid ${T.border}; box-shadow:0 32px 80px rgba(13,15,20,.22), 0 8px 24px rgba(13,15,20,.12); }
.modal-pdf-box { max-width:900px; height:92vh; }
.modal-close { position:absolute; top:14px; right:14px; width:34px; height:34px; border-radius:50%; border:1.5px solid ${T.border}; background:${T.white}; display:flex; align-items:center; justify-content:center; color:${T.muted}; cursor:pointer; transition:all .15s; z-index:10; }
.modal-close:hover { border-color:${T.red}; color:${T.red}; background:${T.redLight}; }
.modal-img-wrap { height:280px; display:flex; align-items:center; justify-content:center; overflow:hidden; flex-shrink:0; background:${T.bg}; }
.modal-img-wrap img { width:100%; height:100%; object-fit:cover; }
.modal-body { padding:22px 26px 28px; overflow-y:auto; }
.modal-title { font-family:'Poppins', sans-serif; font-size:18px; font-weight:600; color:${T.dark}; line-height:1.35; margin-bottom:14px; }
...
.modal-pdf-header { display:flex; align-items:center; justify-content:space-between; padding:16px 22px 14px; border-bottom:1.5px solid ${T.border}; gap:12px; flex-shrink:0; }
.modal-pdf-viewer { flex:1; overflow:hidden; min-height:0; display:flex; flex-direction:column; }
.modal-action-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 16px; border-radius:12px; border:1.5px solid ${T.border}; background:${T.white}; color:${T.dark}; font-size:12.5px; font-weight:700; transition:all .15s; cursor:pointer; font-family:'Inter', sans-serif; }
.modal-action-btn:hover { border-color:${T.red}; color:${T.red}; background:${T.redLight}; }
.pdf-nav-bar { display:flex; align-items:center; justify-content:space-between; padding:11px 22px; border-top:1.5px solid ${T.border}; background:${T.white}; flex-shrink:0; gap:12px; }
.pdf-nav-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 20px; border-radius:12px; border:1.5px solid ${T.border}; background:${T.white}; color:${T.dark}; font-size:13px; font-weight:700; cursor:pointer; transition:all .15s; font-family:'Inter', sans-serif; }
...
.pip { width:8px; height:8px; border-radius:50%; border:none; background:${T.border}; cursor:pointer; padding:0; transition:all .18s; flex-shrink:0; }
...
/* ── Responsive ── */
@media (max-width:1100px) { .sidebar-col { width:210px; } .right-ad-col { display:none; } }
@media (max-width:960px)  { .sidebar-col { display:none !important; } .filter-btn { display:inline-flex; } .top-ad-container { min-height:70px; } }
